...
func (repo *LocalityRepository) CreateLocality(locality models.Locality) (models.Locality, error) {
	...
	row := repo.db.QueryRow(
		"select p.id from countries c join provinces p on c.id = p.id_country_fk where country_name = ? and province_name = ?",
		locality.CountryName,
		locality.ProvinceName,
	)
	if err := row.Err(); err != nil {
		return models.Locality{}, error_management.HandleRepositoryError(
			repository.ErrProvinceNotFound,
			err,
		)
	}
	...
	if err != nil {
		...
		return models.Locality{}, error_management.HandleRepositoryError(
			repository.ErrLocalityRepositoryGeneric,
			err,
		)
	}

	result, err := repo.db.Exec(
		"INSERT INTO localities (locality_name,province_id,zip_code) values (?,?,?)",
		locality.Name,
		provinceId,
		locality.ZipCode,
	)
	...
	if err != nil {
		return models.Locality{}, error_management.HandleRepositoryError(
			error_management.HandleLocalityRepositoryError(err),
			err,
		)
	}
...
func (repo *LocalityRepository) GetSellersByLocality(
	localityId int,
) (models.LocalitySellers, error) {
	var localitySellers models.LocalitySellers

	row := repo.db.QueryRow(
		"SELECT l.id, l.zip_code, l.locality_name, count(s.id) as seller_count FROM localities l LEFT JOIN sellers s ON s.locality_id = l.id WHERE l.id = ? GROUP BY l.id, l.zip_code, l.locality_name",
		localityId,
	)
	if err := row.Err(); err != nil {
		return models.LocalitySellers{}, error_management.HandleRepositoryError(
			error_management.HandleLocalityRepositoryError(err),
			err,
		)
	}
	...
	err := row.Scan(
		&localitySellers.LocalityId,
		&localitySellers.ZipCode,
		&localitySellers.Name,
		&localitySellers.SellersCount,
	)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return models.LocalitySellers{}, error_management.HandleRepositoryError(
				repository.ErrLocalityNotFound,
				err,
			)
		}
		return models.LocalitySellers{}, error_management.HandleRepositoryError(
...
func (repo *LocalityRepository) GetCarriersByLocality(id int) (models.LocalityCarriers, error) {
	...
	if err := row.Err(); err != nil {
		return models.LocalityCarriers{}, error_management.HandleRepositoryError(
			repository.ErrLocalityNotFound,
			err,
		)
	}
