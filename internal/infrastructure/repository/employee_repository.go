...
func (_repository *EmployeeRepository) GetEmployees() (employees map[int]models.Employee, err error) {
	...
	rows, err := _repository.db.Query(
		"SELECT id, card_number_id, first_name, last_name, warehouse_id FROM employees",
	)
	if err != nil {
		return nil, error_management.HandleRepositoryError(
			repository.ErrEmployeeInternalServerError,
			err,
		)
	}
	defer rows.Close()
...
func (_repository *EmployeeRepository) GetEmployeeById(
	employeeId int,
	...
	if row.Err() != nil {
		return models.Employee{}, error_management.HandleRepositoryError(
			repository.ErrEmployeeInternalServerError,
			err,
		)
	}
...
func (_repository *EmployeeRepository) generateId() (int, error) {
	...
	if err != nil {
		return 0, error_management.HandleRepositoryError(
			repository.ErrEmployeeInternalServerError,
			err,
		)
	}
...
func (_repository *EmployeeRepository) GetInboundOrdersCountByEmployeeId(
	employeeId int,
) (employee models.EmployeeReportInboundOrders, err error) {
	row := _repository.db.QueryRow(
		"SELECT employees.id, employees.card_number_id, employees.first_name, employees.last_name, employees.warehouse_id, COUNT(inbound_orders.id) AS inbound_orders_count FROM employees LEFT JOIN inbound_orders ON employees.id = inbound_orders.employe_id WHERE employees.id = ? GROUP BY employees.id",
		employeeId,
	)
	if row.Err() != nil {
		return models.EmployeeReportInboundOrders{}, error_management.HandleRepositoryError(
			repository.ErrEmployeeInternalServerError,
			err,
		)
...
func (_repository *EmployeeRepository) GetInboundOrdersCountAllEmployees() (employees []models.EmployeeReportInboundOrders, err error) {
	rows, err := _repository.db.Query(
		"SELECT employees.id, employees.card_number_id, employees.first_name, employees.last_name, employees.warehouse_id, COUNT(inbound_orders.id) AS inbound_orders_count FROM employees LEFT JOIN inbound_orders ON employees.id = inbound_orders.employe_id GROUP BY employees.id",
	)
	if err != nil {
		return nil, error_management.HandleRepositoryError(
			repository.ErrEmployeeInternalServerError,
